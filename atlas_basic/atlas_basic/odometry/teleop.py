...
if __name__ == "__main__":
    ...
    pub = node.create_publisher(Twist, '/vel_cmd', 10)
    ...
    rate = 10
    servo = node.create_rate(rate, node.get_clock())
    dt    = 1.0 / rate

    # Report.
    node.get_logger().info("Teleop nominal fwd = %6.3f m/s, spin = %6.3f rad/sec"
                  % (vnom, wnom))
    node.get_logger().info("Teleop sending every %f sec..." % dt)
    ...
    node.get_logger().info("Stopping transmissions.")
    node.destroy_node()
    rclpy.shutdown()
    thread.join()
