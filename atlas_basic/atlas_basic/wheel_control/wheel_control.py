...
wheel_control.py
   ...
   Publish:    /wheel_state            sensor_msgs/JointState
               ...
               TF base -> lwheel       geometry_msgs/TransformStamped
               TF base -> rwheel       geometry_msgs/TransformStamped
...
class WheelControl(Node):
    ...
    def __init__(self, i2cbus):
        ...
        self.tfpub_base_to_lwheel = tf2_ros.TransformBroadcaster(self)
        self.tfpub_base_to_rwheel = tf2_ros.TransformBroadcaster(self)
        ...
        self.lwheelmsg = TransformStamped()
        self.rwheelmsg = TransformStamped()
        self.newmsg = False
    ...
    def cb_timer(self):
        ...
        self.actmsg.name         = ['lwheel', 'rwheel', 'gyro']
        ...
        # Publish the transforms for viewing
        # self.lwheelmsg = TransformStamped()
        # self.lwheelmsg.header.stamp            = now.to_msg()
        # self.lwheelmsg.header.frame_id         = 'base'
        # self.lwheelmsg.child_frame_id          = 'lwheel'
        # self.lwheelmsg.transform.rotation      = Quaternion(w=0.0, x=0.0, y=math.sin(theta_L/2), z=math.cos(theta_L/2))
        # self.rwheelmsg = TransformStamped()
        # self.rwheelmsg.header.stamp            = now.to_msg()
        # self.rwheelmsg.header.frame_id         = 'base'
        # self.rwheelmsg.child_frame_id          = 'rwheel'
        # self.rwheelmsg.transform.rotation      = Quaternion(w=0.0, x=0.0, y=math.sin(theta_R/2), z=math.cos(theta_R/2))

        # Publish the desired wheel state /wheel_desired
        self.desmsg = JointState()
        self.desmsg.header.stamp = now.to_msg()
        self.desmsg.name         = ['lwheel', 'rwheel']
    ...
    def cb_pubtimer(self):
        if (self.newmsg):
            ...
            # self.tfpub_base_to_lwheel.sendTransform(self.lwheelmsg)
            # self.tfpub_base_to_rwheel.sendTransform(self.rwheelmsg)
